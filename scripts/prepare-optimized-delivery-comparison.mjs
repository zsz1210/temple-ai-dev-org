...
export function matrixPlan(selection = "both", workItemId = "WI-0179") {
  need(["both", "terra", "screen"].includes(selection), "matrix-selection");
  need(/^WI-[0-9]{4,}$/.test(workItemId), "matrix-work-item");
  const all = [
  ...
  const pairs = (selection === "screen" ? all.slice(0, 2) : all.filter(p => selection === "both" || p.model === "gpt-5.6-terra")).map(p => ({ ...p, reasoning_effort: "medium" }));
  return {
    schema_version: schema, work_item_id: workItemId, selection, pairs,
...
export function validateMatrix(matrix) {
  ...
  need(digest(matrix.plan) === digest(matrixPlan(matrix.plan?.selection, matrix.plan?.work_item_id)), "matrix-plan-drift");
...
export function validateMatrixApproval(approval, matrix) {
  ...
  need(approval?.schema_version === "temple.optimized-delivery-matrix-approval/v1" && approval.status === "approved" &&
    approval.work_item_id === matrix.plan.work_item_id && approval.matrix_sha256 === digest(matrix), "matrix-approval-binding");
...
export async function prepareMatrix({ matrixRoot, sourceRoot, selection = "both", workItemId = "WI-0179", readinessReview }) {
  ...
  for (const pair of plan.pairs) {
    ...
    const protocol = createProtocol(manifest, {
      work_item_id: plan.work_item_id, model: pair.model, reasoning_effort: pair.reasoning_effort,
...
async function inspectMatrixPairs(matrixRoot, matrix) {
  ...
  for (let index = 0; index < matrix.pairs.length; index++) {
    ...
    need(protocol.work_item_id === matrix.plan.work_item_id && protocol.source_sha256 === matrix.source_sha256 &&
...
export async function runMatrix({ matrixRoot, approval, onProgress = () => {} }) {
  ...
  try {
    for (let index = 0; index < pairs.length; index++) {
      ...
      const pairApproval = {
        schema_version: "temple.delivery-pair-approval/v2", status: "approved", work_item_id: matrix.plan.work_item_id,
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  ...
  else if (operation === "prepare" && [4, 5].includes(values.length)) console.log(JSON.stringify(await prepareMatrix({
    sourceRoot: values[0], matrixRoot: values[1], selection: values[2], readinessReview: await read(values[3]), workItemId: values[4]
  }), null, 2));
  ...
  else throw new Error("Usage: plan [both|terra|screen]; prepare SOURCE NEW_MATRIX_ROOT both|terra|screen READINESS_JSON [WI-ID]; run MATRIX_ROOT APPROVAL_JSON");
}
