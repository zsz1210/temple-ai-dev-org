...
export function stageRequests({ root, arm, stage, protocol, threadId = "schema-preview" }) {
  ...
  const instruction = `${processGuide}\nRead BRIEF.md for the complete product contract. ${stage === "build" ? "Implement order.mjs, add meaningful test/added.test.mjs, preserve all supplied files, run node --test test/*.test.mjs, commit only implementation and tests, then write DELIVERY.json and HANDOFF.md. DELIVERY.json must have candidate_revision, test_command, test_exit_code, decision (delivered), summary and unresolved. HANDOFF.md explains exact candidate, test result and next verifier obligations. Commit evidence separately if desired." : "Fresh verification only: read DELIVERY.json and HANDOFF.md, resolve and independently test the exact candidate. Do not modify order.mjs, tests or product documentation. Write VERIFICATION.json with candidate_revision, test_command, test_exit_code, decision (accept or reject), summary and unresolved. Reject defective products. No second feature."}\nReturn a structured record matching the evidence file's candidate_revision, test_command, test_exit_code, decision and unresolved exactly; summary wording may differ. Run the complete product tests yourself against the exact candidate during this stage; the Verifier must not substitute the Builder's result. Within this turn, permitted product edit/test iterations are allowed. Rerun after any change to implementation, tests, relevant configuration, dependencies or test inputs. Writing only delivery, handoff or verification evidence does not require another product-test run when the tested inputs and exact candidate binding remain unchanged; record your current observed exit code. Still perform required evidence validation. If relevance or candidate binding is uncertain, do not reuse the result. The Verifier must reject unexpected product drift rather than edit forbidden files. Exact test_command: node --test test/*.test.mjs. No hidden tests or expected answers are available to you.`;
  const developer = "One bounded local actor turn; no subagents, network, external tools, installations, user questions, extra actor-turn retries or fallback. No retries means no new actor turn or experiment retry, not a ban on tests: each actor independently runs all product tests on its exact candidate. Rerun after permitted changes to implementation, tests, relevant configuration, dependencies or test inputs. Evidence-only writes do not trigger product retesting if those inputs and the candidate binding are unchanged; evidence validation still applies. Uncertain relevance or binding prevents result reuse. Respect the stage's write scope; a Verifier rejects unexpected product drift rather than repairing it. Truthfully record your own observed result. Stay in this repository; never read memories, other repositories, coordinator files or parent paths. Use apply_patch for writes, node --test test/*.test.mjs for tests, node ./templew.mjs for Temple, and simple git/read commands. One shell command per call; no pipes, redirects, substitutions, environment changes or arbitrary scripts. The verifier may write only VERIFICATION.json and permitted .ai-org evidence. Do not edit supplied public tests, package.json, BRIEF.md, README.md or workflow instructions. Git hooks/configuration and symlinks are forbidden.";
...
export function deliveryProcessContract() {
  ...
  return {version:"delivery-process/v9",templates,command_policy:commandPolicyContract,fixture_sha256:digest(fixture),output_schema:completionSchema,model_and_effort_are_parameters:true,temporary_environment:"stage-private-TMPDIR-in-sibling-runtime-directory; retained-separately-unsealed-operational-scratch",git_metadata:"explicit-assigned-repository-only; configuration-and-hooks-integrity-checked"};
}
