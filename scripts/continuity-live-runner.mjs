...
export async function isolatedOracleExecutor(runtime,cwd,binary,args,options,{providerFactory=createJsonRpcProcess}={}) {
  ...
  try {
    ...
    const r=await c.request('command/exec',{command:['/usr/bin/env','-i',...Object.entries(environment).map(([k,v])=>`${k}=${v}`),binary,...args],
      cwd,permissionProfile:ownedProfile,timeoutMs:options.timeout,outputBytesCap:options.maxBuffer},10000);
  ...
  finally {if(c)try{await c.close();}catch{const error=primary??Error('oracle-cleanup-unconfirmed');error.instrumentFailure=true;error.retainScratch=true;error.cleanupFailure='oracle-cleanup-unconfirmed';throw error;}}
}
