...
## 讓 Token 使用與模型選擇逐步有依據

當執行環境能提供用量資料時，Temple 會把每項工作的 Token 使用量，和能確認的模型、職位、工作階段與結果連在一起。這些紀錄可以幫忙找出用量變化的原因，也可能形成一個信心較低的觀察候選；但一般開發工作難度不一，不能直接拿來判定哪個模型比較好。

專案也可以登錄「配對評估」：讓不同模型處理完全相同的案例，並固定工作類型、原始版本與品質標準。Temple 會先確認品質是否達標，再比較 Token、處理時間、重工次數，以及需要人工介入的程度。只有品質門檻和專案事先設定的統計條件都通過，才會提出一項有依據、但不會自行執行的模型建議；品質不合格的模型，不會因為 Token 較少就被選中。

Temple 不會自行呼叫模型來跑這些評估，也不會替工作切換模型、改寫專案規則，或把校準資料自動傳到其他專案。目前 Temple 自己的版本庫尚未登錄配對評估資料，因此也還沒有針對自身開發得出真正的模型建議。自動模型路由仍是後續能力，必須另外完成執行、授權、安全退回與實際驗證機制。

計算方式、隱私界線、模型建議與自動化條件，請參考 [Token 效率與模型路由（英文）](docs/operations/token-efficiency-and-model-routing.md)。
...
| **功能已實作，驗證範圍仍有限** | Collaborative、High-Assurance、安全的並行規劃、服務狀態、逐工作項目的 Token／模型用量觀察、信心較低的觀察候選、由專案登錄資料後產生的配對評估建議、唯讀的外部追蹤、多專案總覽，以及本機控制功能。這些能力已通過自動測試或小範圍驗證，但還不能據此判定適合任何團隊直接用於正式流程。 |
| **尚未完成驗證** | 大型團隊與多台電腦的實際運作、正式環境的自動監測或修復、無人值守的外部寫入、自動模型路由、跨專案自動學習、完成設定的語意搜尋、有法規或稽核要求的環境，以及在大型企業全面使用。 |
...
| 了解 Token 使用、模型校準與自動路由的界線 | [Token 效率與模型路由（英文）](docs/operations/token-efficiency-and-model-routing.md) |
