...
const schemas = {};
for (const name of ['ThreadStartParams', 'TurnStartParams', 'ThreadResumeParams', 'ItemStartedNotification', 'ItemCompletedNotification', 'ThreadTokenUsageUpdatedNotification', 'TurnStartedNotification', 'TurnCompletedNotification']) {
...
async function replay(events, { resume = {}, interruptFailure = false } = {}) {
...
test('confirmed activity binds one child and replays quarantined events', () => {
  ...
  assert.equal(tracker.actors.get('c').acquisitionBasis, 'activity-resume');
  assert.equal(tracker.pending.length, 0);
  assert.equal(tracker.report().aggregate_operational_tokens, null);
});

test('late spawn corroborates an activity-resume binding once', () => {
  const tracker = makeTracker();
  tracker.event(event('item/completed', 'p', activity('c')));
...
test('executor acquires an activity-only helper and captures replayed evidence', async () => {
  ...
  assert.equal(result.trace.actors.find(actor => actor.role === 'helper').acquisition_basis, 'activity-resume');
