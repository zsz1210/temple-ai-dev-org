...
import {NativeTracker} from '../WI-0196/native-tracker.mjs';
...
export async function runSubject({fixture:f,protocol:p,contract,deadline,aggregateBefore=0,providerFactory=createJsonRpcProcess}){
 ...
 const resumeChild=(child,bindFromActivity)=>{
 ...
 const process=(event,captureOnly=false)=>{
  try{
   ...
   if(!captureOnly){
    ...
    flushPendingEvidence();
    if(v?.item?.type==='subAgentActivity'&&v.threadId===tid&&typeof v.item.agentThreadId==='string'&&!tracker.hasActor(v.item.agentThreadId))resumeChild(v.item.agentThreadId,true);
   }
   if(!captureOnly&&v?.item?.type==='collabAgentToolCall'&&v.item.tool==='spawnAgent'&&method==='item/completed'){
    ...
    for(const child of tracker.children)resumeChild(child,false);
   }
