# Operating obligation map

Baseline: `4c606f7113600caeec04daf29477daa17b07409c`. This is an authored-contract
review, not evidence that a model has read or understood instructions.

| Prior obligation (formerly repeated across AGENTS/TEMPLE/Skill/Lean) | Retained owner |
| --- | --- |
| Native/project entrypoints; whole authority; no digest-as-reading; required nested/unselected sources | TEMPLE opening; AGENTS entry; Lean read/claim |
...
| Exact managed entries only; project-owned preservation; pinned CLI/bootstrap mismatch | TEMPLE authority; AGENTS entry; Skill opening |
| Integration policy confirmation/deferral, distributed coordination, no imposed GitHub Flow or merge authority | TEMPLE authority |
| Approved current scope/supporting specs; native digest pins; gate-evidence distinction; reconciliation; contract refs vs shared contracts | TEMPLE product-authority row |
| Context map concise; active Practices/validated Lessons; Learning CLI; no automatic promotion | TEMPLE context row |
| UI owner, mode, approved preview and runtime evidence; no-UI condition and vendor independence | TEMPLE UI row |
| High-Assurance risk, sponsor, exact evidence, distinct principals, rollback and release separation | TEMPLE High-Assurance row; unchanged assurance reference |
| Tracker configuration, company/local/runtime distinction, no credentials or implicit write-back/gate completion | TEMPLE tracker row |
| Optional Observer/Console, unsupported unknown, projections not assurance | TEMPLE Console row |
...
| Lean closeout vs formal assurance, Release Gate evidence/rollback/approval | Skill finish; unchanged assurance reference |
| Pilot scope and observable stop; no implied next experiment/task/dependency/merge/publish/deploy | TEMPLE completion; AGENTS final paragraph; Lean stop |

## What is actually removed

Repeated policy explanations and duplicate completion prose. The same four sources
are still read whole on a cold Lean entry. The number of required claim/finish
calls, canonical records and project test gates is unchanged. We do not count the
already available compact navigation instead of full entry JSON as this treatment.
Existing user-owned instructions can increase the read set and remain authoritative.
