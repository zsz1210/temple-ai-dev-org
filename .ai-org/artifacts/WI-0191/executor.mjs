...
export async function runSubject({fixture:f,protocol:p,contract,deadline,aggregateBefore=0,providerFactory=createJsonRpcProcess}){
 ...
 let conn,tracker,tid,turn,stop,closing=false,answer=null,cleanup=null;const early=[],observations=[],messages=[],pendingEvidence=[],subscriptions=new Set(),pendingSubscriptions=[];let wake;const done=new Promise(r=>wake=r);
 const fail=reason=>{stop??=reason;wake();};
 const process=(event,captureOnly=false)=>{
  try{
   const {method,params:v}=event;
   const schema={'item/started':'ItemStartedNotification','item/completed':'ItemCompletedNotification','thread/tokenUsage/updated':'ThreadTokenUsageUpdatedNotification'}[method];
  ...
  }catch(e){fail(e.message.startsWith('schema-')?e.message:'event-contract-violation');}
 };
 ...
 try{
  ...
  conn=providerFactory('codex',representativeAppServerArguments,{cwd:f.target,env:subprocessEnvironment({TEMPLE_CLI_PATH:path.join(f.source,'bin/temple.mjs'),TMPDIR:path.join(f.target,'.scratch')}),
   onNotification:e=>{if(closing)return;if(!tracker){if(early.length>=4000)fail('early-event-cap');else early.push(e);}else process(e);},
 ...
 return {case:f.id,arm:f.arm,status:stop?'stopped':tracker?.report().status??'incomplete',stop_reason:stop,elapsed_ms:Date.now()-start,requested_model:p.model,requested_effort:p.effort,
  observations,messages,cleanup,trace:tracker?.report()??null,answer:typeof answer==='object'?answer:null,raw_tool_output_retained:false,reasoning_retained:false};
}
