...
The fresh Wave 5B lab was created at `<LOCAL_HOME>/Documents/ChatGPT/temple-wave-5b-lab`. Four candidate repositories are clean and pinned to the frozen case fixtures and integrated Temple revision. The exact installed Codex CLI and all ten required App Server schema digests match the protocol preflight; model discovery confirms Luna Medium.

The first setup iteration was rejected because evaluator-only fields appeared in the candidate validation manifest. The failed lab was preserved as `<LOCAL_HOME>/Documents/ChatGPT/temple-wave-5b-lab.preflight-invalid-20260903`; the protocol now separates candidate, evaluator, and combined limits. The second clean lab passes every offline candidate check except the intentionally absent owner approval.
