...
import { requests, runSubject, sha } from '../WI-0196/executor.mjs';
...
export async function bindings() {
  const result = {};
  for (const name of ['design.md', 'protocol.json', 'preflight.mjs', 'runner.mjs']) {
    result[`.ai-org/artifacts/WI-0197/${name}`] = sha(await fs.readFile(path.join(here, name)));
  }
  for (const name of ['event-policy.mjs', 'executor.mjs', 'native-tracker.mjs']) {
