...
Publication audit reports two blocked-pattern findings in unchanged main fixtures: WI-0192 and WI-0193 measurement.test.mjs line 31 use the fictional `<LOCAL_HOME>/key` string with a sentinel to test redaction. They are not a maintainer path or credential. This is a manual fixture qualification, not a claim that the automated audit is all-green. The 68 unchanged binary review items retain the user's prior image-review exemption. Added-line scan found no actual home-path or configured credential-pattern matches; this bounded scan is not a universal secret guarantee.
