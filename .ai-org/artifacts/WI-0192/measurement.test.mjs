...
test('error data contains bounded categories and digest only', () => {
  const error=boundedNativeError({status:'failed',agentsStates:{child:{status:'errored',message:'private <LOCAL_HOME>/key SECRET'}}});
...
test('successor live execution is disabled even with supplied filenames', async () => {
  await assert.rejects(execute('/missing','/missing','/missing'),/successor-live-disabled/);
});
test('archived authorization has execution-time hash', async () => {
  const original=await fs.readFile(new URL('./authorization-original.txt',import.meta.url));
