import { supportOutcome, validTestReceipt } from './measurement.mjs';
...
export async function gradeCase(f,result,{execute}){
 ...
 if(f.id==='entry-normal'){
  ...
  facts.added_test_exit=result.evaluator_test_receipt?.exit_code??null;
  if(Number.isInteger(facts.added_test_exit))require(facts.added_test_exit===0,'submitted-tests-failed');
  facts.subject_test_execution=(result.observations??[]).some(x=>x.thread==='parent'&&x.product_test_invocation==='exact-literal-at-fixture-root'&&x.exit_code===0)?'observed':'unknown';
  require(validTestReceipt(result.evaluator_test_receipt),'evaluator-test-receipt-missing');
  ...
  const support=supportOutcome(result);facts.support=support;
  require(support!=='false-acceptance','helper-promoted-to-acceptance');
  if(support==='unavailable')return {status:'unmeasurable',failures,facts,independent_review_required:true,transient_shell_writes_proven_absent:false};
  ...
  require(a?.decision==='reported','support-answer-not-reported');
