import { classifyItem } from '../WI-0196/event-policy.mjs';
...
export class NativeTracker {
  ...
  accept({ method, params } = {}) {
    ...
    if (method === 'turn/completed') {
      ...
      if(params.turn.status==='completed') demand([...actor.items.values()].every(item => item.done), 'unfinished-item');
  ...
  report() {
    const actors = [...this.actors].map(([id, actor]) => ({
      ...
      unfinished_items: [...actor.items.values()].filter(item=>!item.done).length,
      usage: actor.usage,
