import { EventJournal, assertItemCoverage } from '../WI-0196/event-policy.mjs';
...
export function requests(f,p){
 ...
 return {thread:{model:p.model,cwd:f.target,approvalPolicy:'never',sandbox:'workspace-write',config:{model_reasoning_effort:p.effort},
  developerInstructions:`One bounded local scenario. Read AGENTS.md, TEMPLE.md and ACTOR.json. Use only the assigned repository and pinned launcher. No network, installations, other repositories, memory, user questions, retries, fallback or later responsibilities. Product/test writes only for entry-normal. Use apply_patch. Return requested JSON.
Delegation authority is role-scoped: only the top-level parent may act on a root user request that explicitly authorizes one helper. A native subagent receiving a delegated brief is a leaf and must do that brief directly, never invoke collaboration tools or delegate further. Parent orchestration instructions do not apply to leaf agents. Parent must use fork_turns "none" and a self-contained read-only leaf brief that explicitly forbids delegation.`,...wave5ThreadIsolation(f.target)},
  turn:{threadId:'pending',input:[{type:'text',text:(support?`ROOT PARENT ONLY: request exactly one native helper with model ${p.model}, effort ${p.effort}, fork_turns none. Its brief must identify it as a leaf, forbid all delegation, include the absolute repository path and the bounded question. Await its findings, then return the final JSON.\n\n`:'')+f.prompt}],cwd:f.target,approvalPolicy:'never',sandboxPolicy:sandbox,model:p.model,effort:p.effort,outputSchema:caseSchema(f.id)}};
}
export async function runSubject({fixture:f,protocol:p,contract,deadline,aggregateBefore=0,providerFactory=createJsonRpcProcess}){
 if(providerFactory===createJsonRpcProcess)throw Error('generation-disabled-pending-native-guard-validation');
 ...
 const flushPendingEvidence=()=>{
  for(let i=0;i<pendingEvidence.length;){const event=pendingEvidence[i];if(tracker.hasActor(event.params.threadId)){pendingEvidence.splice(i,1);process(event,true);}else i++;}
 };
 const maybeDone=()=>{
  if(!tracker)return;
  const r=tracker.report();
  if(r.actors.every(a=>a.terminal)&&r.observed_children===tracker.maxChildren&&r.unbound_activity_ids.length===0)wake();
 };
 const acquireChild=(child,bindFromActivity)=>{
  // Observation of an in-memory child is not resumption of a disk rollout.
  if(subscriptions.has(child))return;
  subscriptions.add(child);
  const args={threadId:child,includeTurns:false};validate('ThreadReadParams',args);
  const pending=conn.request('thread/read',args,10000).then(response=>{
   validate('ThreadReadResponse',response);
   const t=response.thread;
   demand(t.id===child,'child-metadata-id-mismatch');
   demand(t.parentThreadId===tid,'child-metadata-parent-mismatch');
   demand(t.ephemeral===true,'child-metadata-retention-mismatch');
   demand(realpathSync(t.cwd)===realpathSync(f.target),'child-metadata-cwd-mismatch');
   demand(t.model===p.model,'child-metadata-model-mismatch');
   demand(t.reasoningEffort!=null,'child-metadata-effort-unconfirmed');
   demand(t.reasoningEffort===p.effort,'child-metadata-effort-mismatch');
   ...
   fail(named?error.message:'child-metadata-failed');
  });
 ...
 const process=(event,captureOnly=false)=>{
  try{
   ...
   if(!captureOnly){
    tracker.event(event);
    if(v?.item&&!tracker.hasActor(v.threadId)){demand(pendingEvidence.length<64,'pending-evidence-cap');pendingEvidence.push(event);}
    flushPendingEvidence();
    // Wait for the native child's turn-start before asking for live metadata.
    // Either ordering (activity first or child event first) is supported.
    for(const child of tracker.activityHints){
     const started=tracker.pending.some(e=>e.params.threadId===child&&e.method==='turn/started')||tracker.actors.get(child)?.turn;
     if(started)acquireChild(child,!tracker.hasActor(child));
    }
   }
   if(!captureOnly&&v?.item?.type==='collabAgentToolCall'&&v.item.tool==='spawnAgent'&&method==='item/completed'){
    ...
    for(const child of tracker.children)if(tracker.actors.get(child)?.turn)acquireChild(child,false);
   }
   if(v?.item?.type==='fileChange'&&tracker.hasActor(v.threadId)){
   ...
   if(v?.item?.type==='agentMessage'&&method==='item/completed'&&tracker.hasActor(v.threadId)){
   ...
   if(v?.item?.type==='commandExecution'&&method==='item/completed'&&tracker.hasActor(v.threadId)){
   ...
   if(actors.some(a=>a.terminal&&a.terminal!=='completed'))fail('actor-terminal-failure');
 ...
 try{
  ...
  if(!stop){demand(tracker.report().status==='observed-complete','incomplete-lifecycle');const parsed=JSON.parse(answer);demand(ajv.compile(req.turn.outputSchema)(parsed),'output-schema');answer=parsed;}
 ...
 return {case:f.id,arm:f.arm,status:stop?'stopped':tracker?.report().status??'incomplete',stop_reason:stop,elapsed_ms:Date.now()-start,requested_model:p.model,requested_effort:p.effort,
  metadata_authority:'configured-route-not-per-turn-execution',event_journal:journal.report(),observations,native_errors,messages,cleanup,trace:tracker?.report()??null,answer:typeof answer==='object'?answer:null,raw_tool_output_retained:false,reasoning_retained:false};
}
