...
// Usage: node .ai-org/artifacts/WI-0203/analyze.mjs PRIVATE_MATRIX_ROOT
import fs from "node:fs/promises";
import path from "node:path";
import assert from "node:assert/strict";
import { digest, retainedArtifactDigest, operationStatistics, costBreakdown } from "../../../scripts/delivery-control-pair.mjs";
...
for (const planned of matrix.plan.pairs) {
  ...
  const stages = run.stages.map(stage => {
    ...
    assert.deepEqual(costBreakdown(stage), stage.cost_breakdown);
    ...
    return { ...pick(stage, ["arm", "stage", "status", "stop_reason", "requested_model", "acknowledged_model", "model_acknowledgement_basis", "requested_effort", "observed_thread_effort", "effective_turn_effort", "usage", "usage_finality", "total_elapsed_ms", "turn_elapsed_ms", "quality_passed", "quality_reason", "candidate_revision", "completion_agreement", "workflow", "treatment", "public_tests", "oracle", "retry_count", "fallback_count", "operations", "cost_breakdown", "command_started_count", "command_completed_count", "unmatched_command_starts", "unmatched_patch_starts"]),
