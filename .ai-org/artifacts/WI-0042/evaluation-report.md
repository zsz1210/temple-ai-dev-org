...
- **Exact private bind:** pass. The live process owns `<PRIVATE_IPV4>:41741` and `127.0.0.1:56635`; no wildcard listener exists. Automated address cases reject wildcard, loopback, link-local, carrier-grade NAT, multicast, documentation, public, hostname, URL, and IPv6 inputs.
...
Physical tablet connectivity is intentionally an owner confirmation rather than an automated claim. The Mac Mini has proved the same-LAN route and exact listener; the owner should now open `http://<PRIVATE_IPV4>:41741` from the tablet on home Wi-Fi.
