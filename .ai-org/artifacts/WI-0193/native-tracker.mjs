...
export class NativeTracker {
 constructor({parent,turn,model,effort,maxChildren=0,maxEvents=4000}){
  ...
  this.actors=new Map([[parent,{turn,terminal:null,usage:null,items:new Map(),calls:0,bytes:0}]]);this.pending=[];this.count=0;this.children=new Set();this.stop=null;
 }
 ...
 accept({method,params:p}={}){
  ...
  const tool=['commandExecution','fileChange','collabAgentToolCall'].includes(i.type);
  if(!tool){demand(['agentMessage','reasoning','plan','userMessage','contextCompaction'].includes(i.type),'unknown-item');return;}
  ...
  if(i.type==='collabAgentToolCall'){
   ...
   if(i.tool==='spawnAgent'){
    if(i.status==='failed'){start.done=true;this.stop??='native-spawn-failed';return;}
 ...
 report(){
  ...
  return {actors,unknown_event_count:this.pending.length,expected_children:this.maxChildren,observed_children:this.children.size,
   status:this.stop?'stopped':this.pending.length||this.children.size!==this.maxChildren||actors.some(a=>a.terminal!=='completed'||!a.usage)?'incomplete':'observed-complete',
