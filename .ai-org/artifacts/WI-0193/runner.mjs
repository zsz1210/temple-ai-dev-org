...
import { stopReasonFor, executeProductTests } from './measurement.mjs';
...
export async function prepare(){
 ...
 const first={target:path.join(f.lab,'support-read-before'),source:path.join(f.lab,'before'),id:'support-read',arm:'before'};
 ...
 return {lab:f.lab,protocol_sha256:sha(seal),scenarios:cases.length,subject_turn_cap:8,sandbox:seal.sandbox,live_ready:false,reason:seal.live_gate,model_calls:0};
}
export async function execute(lab,approvalPath,reviewPath){
 ...
 for(const c of seal.cases){
  ...
  if(c.id==='entry-normal')result.evaluator_test_receipt=await executeProductTests(cmd=>sandboxCommand(f,cmd,{readonly:true}));
  result.case_grade=await gradeCase(f,result,{execute:cmd=>sandboxCommand(f,cmd,{readonly:true})});
  result.quality_status=result.case_grade.status==='unmeasurable'?'unmeasurable':result.case_grade.status==='failed'?'failed-automatic-check':'human-trace-review-required';
