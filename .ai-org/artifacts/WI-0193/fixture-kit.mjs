...
export async function prepareSources(){
 const lab=await fs.mkdtemp(path.join(os.tmpdir(),'temple-wi0193-'));
...
export async function prepareCase(lab,arm,id){
 ...
 if(completed){
  await fs.writeFile(path.join(target,'added.test.mjs'),"import assert from 'node:assert/strict'; import {normalizeTags} from './app.mjs'; const input=Object.freeze([' A ', '', 'a']); assert.deepEqual(normalizeTags(input),['a']); assert.throws(()=>normalizeTags(['a',1]),TypeError);\n");
  const testEnv={...process.env};delete testEnv.NODE_TEST_CONTEXT;
  const test=spawnSync(process.execPath,['--test','app.test.mjs','added.test.mjs'],{cwd:target,encoding:'utf8',env:testEnv});if(test.status!==0||!/tests 2\b/.test(test.stdout))throw Error('reference-failed');
  await fs.writeFile(path.join(target,'docs/test.md'),`# Observed product test\nnode --test app.test.mjs added.test.mjs\nExit ${test.status}\n`);
