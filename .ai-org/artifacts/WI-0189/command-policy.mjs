// WI-0189 experiment-local policy: module ownership and four test paths.
...
function gitCommand(args, state, context) {
  ...
  if (name === "add") {
    ...
    const owned = context.writePaths;
...
function nodeCommand(tokens, state, context) {
  ...
  if (tests.length === 1 && tests[0] === "test/*.test.mjs") {
    need(tokens[1].glob && context.cwd === context.root, "test-command-boundary"); safePath("test/public.test.mjs", context); for (const name of ["validate","summarize","render"]) safePath("test/"+name+"-added.test.mjs", context); state.operation = "product-tests-all"; return;
  }
  need(tests.length > 0 && tests.length <= 4 && new Set(tests).size === tests.length, "test-command-boundary");
  need(tests.every(v => ["test/public.test.mjs", "test/validate-added.test.mjs", "test/summarize-added.test.mjs", "test/render-added.test.mjs"].includes(v)) && context.cwd === context.root, "test-command-boundary");
  tests.forEach(value => safePath(value, context)); if (tests.length === 4) state.operation = "product-tests-all";
}
