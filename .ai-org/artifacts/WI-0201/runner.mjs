...
export async function bindings() {
  const result = {};
  for (const name of ['design.md', 'protocol.json', 'preflight.mjs', 'runner.mjs', 'executor.mjs', 'native-tracker.mjs']) {
    result[`.ai-org/artifacts/WI-0201/${name}`] = sha(await fs.readFile(path.join(here, name)));
  }
  for (const name of ['event-policy.mjs']) {
    result[`.ai-org/artifacts/WI-0196/${name}`] = sha(await fs.readFile(path.join(here, '../WI-0196', name)));
  }
  for (const name of ['fixture-kit.mjs', 'grading.mjs', 'measurement.mjs', 'runner.mjs']) {
...
export async function providerContract() {
  ...
  try {
    ...
    for (const name of ['TurnStartedNotification', 'TurnCompletedNotification', 'ThreadStartResponse', 'ThreadResumeResponse', 'ThreadReadParams', 'ThreadReadResponse', 'TurnStartResponse']) {
...
export function assertProviderContract(contract) {
  const required = ['ThreadStartParams', 'ThreadStartResponse', 'ThreadResumeParams', 'ThreadResumeResponse', 'ThreadReadParams', 'ThreadReadResponse', 'TurnStartParams', 'TurnStartResponse', 'ItemStartedNotification', 'ItemCompletedNotification', 'ThreadTokenUsageUpdatedNotification', 'TurnStartedNotification', 'TurnCompletedNotification'];
