...
- Read `.ai-org/project/usage-policy.json` before recommending or selecting a model, reasoning profile, Credits budget, or calibration action. Routine decisions inside its approved, reversible, local, allowlisted, within-budget envelope are automatic; request approval only for its named exceptions or another governing policy boundary. Its fixed Work Item count is diagnostic coverage, not statistical qualification or routing authority.
...
- Follow the project-owned repository integration record and the authoritative policy it references. The framework does not impose GitHub Flow. If the record is unconfirmed, inspect existing rules first and ask only when the missing integration choice affects the requested work; never infer merge, release, or hosting-setting authority.
