...
## Token 使用量とモデル選択を、実績に基づいて見直す

実行環境から利用量の情報を取得できる場合、Temple は Work Item ごとの Token 使用量を、確認できたモデル、担当 Position、工程、結果と結び付けます。これにより、利用量が増減した要因を調べたり、信頼度の低い観察候補を示したりできます。ただし、通常の開発作業は難易度がそろわないため、この記録だけでモデルの優劣は決めません。

プロジェクトは「同条件評価（matched evaluation）」を登録することもできます。同じタスク形状、同じ評価ケース、同じソースのリビジョン、同じ品質基準で複数のモデルを比べる方法です。Temple はまず品質基準を満たしたかを確認し、その後に Token、所要時間、手戻り、人の介入量を比較します。品質基準と、プロジェクトがあらかじめ定めた統計上の条件を両方満たした場合に限り、根拠付きの読み取り専用提案を示します。品質を満たさない候補が、Token の少なさだけで選ばれることはありません。

Temple 自身がプロバイダーを呼び出して評価を実行したり、作業中のモデルやプロジェクト方針を書き換えたりすることはありません。評価データも、ほかのプロジェクトへ自動共有しません。このリポジトリには、まだ同条件評価のデータが登録されていないため、Temple 自身の開発で使うモデルについて具体的な提案も出していません。自動ルーティングには、実行機能、権限、問題時の復帰方法、実運用での検証を別途用意する必要があります。

計測方法、プライバシー、提案、自動化の境界は、[Token efficiency and model routing（英語）](docs/operations/token-efficiency-and-model-routing.md)で説明しています。
...
| **限定的に検証済み** | Collaborative、High-Assurance、安全な並行計画、プロバイダーの状態、Work Item 単位の Token／モデル利用量の観測、低信頼度の観察候補、プロジェクトが登録した同条件評価に基づく読み取り専用提案、外部トラッカーと複数プロジェクトの読み取り専用表示、ローカル管理画面。自動テストや小規模な検証はありますが、一般的な組織での実運用を証明するものではありません。 |
| **まだ前提にできない** | 大規模チームと複数マシンでの実運用、本番環境の自動監視・修復、無人の外部書き込み、モデルの自動ルーティング、プロジェクトをまたぐ自動学習、設定済みの意味検索、規制対象での利用、幅広い企業導入の実績。 |
...
| Token 使用量とモデル調整の仕組みを理解したい | [Token efficiency and model routing（英語）](docs/operations/token-efficiency-and-model-routing.md) |
