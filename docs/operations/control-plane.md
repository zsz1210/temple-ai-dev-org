...
- **Team** opens on a Position-first **Structure** view, grouped into Product & Experience, Engineering Delivery, and Assurance & Release responsibilities. Each Position names its current assigned Agent, and an Agent filter highlights all responsibilities carried by one teammate without hiding the rest. **Teammates** retains the detailed Agent cards and overlays task-level model evidence as `Active model`, `Last observed`, `Requested model`, or `No model observation`. `Configured` never means online, and the structure is a responsibility map rather than a reporting hierarchy.
