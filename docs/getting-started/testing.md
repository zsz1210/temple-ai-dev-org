...
CI uses one job so GitHub does not round several short jobs into separate billable minutes. Within that job, scope selection, repository checks, schema validation, Doctor, and the selected behavioral lane remain separately named steps. The summary reports both governance and behavior outcomes, and an always-run final step fails the job if any required result failed. A governance failure therefore does not hide the behavioral result.
