...
- The Alpha.29 package uses an explicit inclusion allowlist. The latest dry run contains 309 files, approximately 0.65 MB packed and 2.62 MB unpacked, including localized README diagrams, public ADRs, and the optional Observer runtime and guide while excluding the repository's root `.ai-org`, tests, examples, screenshots, output, integrations, and development scripts. Exact final bytes remain in repository-owned WI-0086 evidence because npm's generated package metadata changes with the candidate commit. The original audit baseline produced 1,906 files and approximately 17.4 MB packed.
...
- The package and installed bootstrap now declare `>=24.0.0`. The WI-0099 working tree passed 272 complete tests under local Node.js `v24.20.0`; exact-commit and clean-package binding remain required before release.
...
| Runtime support | Node.js 24 working-tree verification passed; exact candidate pending | Bind local complete verification and clean-consumer smoke to the final Node.js 24 candidate; later local majors are supplementary signals |
| Real-browser console gate | Working-tree verification passed; exact candidate pending | Installed Chrome must pass four-view semantic and layout checks locally and remain revision-bound release evidence |
