...
- WI-0099 supersedes that active matrix policy while retaining those runs as historical evidence. Follow-up run [`33589295734`](https://github.com/zsz1210/temple-ai-dev-org/actions/runs/33589295734) began another two-job full suite and was cancelled at the repository owner's request to stop unnecessary hosted resource use.
- The first bounded Node.js 24 run [`33591053632`](https://github.com/zsz1210/temple-ai-dev-org/actions/runs/33591053632) finished in seconds but correctly failed Doctor: checkout had become shallow, so historical evidence revisions were unavailable. The corrective workflow retains complete Git history for Doctor while keeping the single job, five-minute ceiling, and bounded test set.
...
- The Alpha.29 package uses an explicit inclusion allowlist. The WI-0099 candidate dry run contains 313 files, including localized README diagrams, public ADRs, and the optional Observer runtime and guide while excluding the repository's root `.ai-org`, tests, examples, screenshots, output, integrations, and development scripts. Exact final bytes remain in repository-owned WI-0086 evidence because npm's generated package metadata changes with the candidate commit. The original audit baseline produced 1,906 files and approximately 17.4 MB packed.
...
- The package and installed bootstrap now declare `>=24.0.0`. Exact candidate `36703f3` passed 272 complete tests and the installed-Chrome gate under local Node.js `v24.20.0` in both Developer and Independent QA runs; clean-package consumer binding remains required before release.
- The installed-Chrome regression gate checks four responsive viewports, semantic navigation, live-state labels, keyboard traversal, reduced motion, console failures, overflow, clipping, and collisions. It uses the pinned development-only `playwright-core` package, downloads no browser, and now runs as local release evidence rather than ordinary hosted CI.
...
| Package boundary | Passed | Explicit `files` allowlist and enforced dry-run manifest passed with 313 files at the WI-0099 candidate |
| Clean installation | Prior candidate passed; WI-0099 repeat pending | Repeat exact tarball version, install, init, re-init, project launcher, and Doctor under Node.js 24 at the final candidate |
...
| Runtime support | Exact Node.js 24 local verification passed; clean consumer pending | Bind clean-consumer smoke to the final Node.js 24 candidate; later local majors are supplementary signals |
| Real-browser console gate | Exact local candidate passed | Installed Chrome passed four-view semantic and layout checks and remains revision-bound release evidence |
| Hosted CI | Old matrix retired; new bounded gate pending first push | One Node.js 24 job must pass repository checks, schema, Doctor, and fast contracts within its five-minute ceiling; it must not run the complete or browser suite |
...
| Maintainer clean-package smoke | Prior candidate passed; WI-0099 repeat pending | The final real tarball must pass the Node.js 24 clean-consumer path with no Doctor warning or failure |
