...
ソースリポジトリと **Alpha.33** は GitHub と npm（`next` チャネル）で公開済みです。配布中のパッケージは初期 Alpha であり、安定版や企業環境での運用保証を意味しません。確認日付きのチャネル情報と検証根拠は [Release readiness（英語）](release-readiness.md) を参照してください。
