...
- **刻意保留的未完成項目：** 不受信任儲存庫的 Provider 信任設計（`WI-0033`）、確認 Provider 能否回報實際模型與推理強度的嚴格驗證（`WI-0064`），以及四個儲存庫的效益演練（`WI-0067`），目前仍處於待決或受阻狀態。它們不阻擋範圍明確、以本機使用為主的公開 Alpha，但在完成前，Temple 不會建議從不受信任的儲存庫啟用 Provider，也不會宣稱已證明效益、節省成本或具備企業級資格。
