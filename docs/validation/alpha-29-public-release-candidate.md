...
## Latest local requalification checkpoint

- Checkpoint `cbd8f2aadd1f5bfe9ef4b961ec24b6fbc3eb5b57` includes WI-0091 and the public conduct/documentation boundary.
- Node.js `v22.23.2` and `v24.20.0` each passed all 270 tests. Installed Chrome `152.0.7977.65` passed four viewports, six primary views, and reduced-motion checks.
...
The Human Principal has approved and tested the private conduct-reporting route, and the repository now contains an enforceable Code of Conduct. The remaining gates are:

1. A genuinely independent new user follows only the public instructions in a new-project or agreed representative path. A maintainer-run temporary-directory smoke remains separate evidence.
2. GitHub branch or ruleset protection, required checks, private vulnerability reporting, secret scanning, and push protection are approved, configured, and verified when repository visibility makes them available.
3. Repository visibility, tag creation, GitHub Release creation, public announcement, and any later npm publication receive separate explicit approval.

Until the remaining gates are satisfied, the public release decision remains **NO-GO** even when the local candidate is green.
