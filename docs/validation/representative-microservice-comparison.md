...
V12 treats the command-item cwd as diagnostic for nested Code Mode commands and keeps authorization bound to the frozen turn environment, exact allowed commands and fixture repository IDs, explicit-path containment, an arm-only write root, and disabled network access. It records the exact installed App Server schema rather than assuming fields present only in newer documentation: the pinned Provider supports `writableRoots` and `networkAccess` but not restricted-read roots. The readiness gate now contains fifteen checks, including the v11 cwd presentation, explicit outside-path rejection, and installed sandbox-schema replay. All fifteen pass with zero Operational Tokens under protocol `94fb522ec92b9f76694d2a1a0d457d9da0e2422ef33d40d0c5463e5ef962dbd0`; preflight has no blocker other than separate exact human approval.
