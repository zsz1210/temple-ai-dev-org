...
- Status: WI-0136 routed-context qualification passed; representative v5 stopped on a runner command-policy mismatch after exact approval, and repaired v6 is frozen and generation-disabled pending separate exact approval
...
Successor v6 is frozen under digest `35deeb5fb60f8f48e818ad6abad7d576d7de976d95159d7a1fbe8ef00baa67c7`. It keeps the v5 task, model routes, context treatment, objective tests, 150,000 Design ceiling, 69,000 per-Build ceiling, 80,000 Integration ceiling, 650,000 candidate ceiling, 100,000 evaluator ceiling, 750,000 combined ceiling, and 45-minute limit. Its runner resolves a relative Git target from the Provider-reported command working directory and accepts it only when the normalized target equals one of the five fixture repository roots. It also interrupts and awaits every sibling Build turn before recording a parallel-wave stop. It does not globally allow `../../`, resume v5, or reuse the v5 approval. Generation remains disabled until verification and exact v6 approval are complete.
