...
7. Keep the pre-recovery task title neutral, for example `Product repository · cold recovery · Independent QA`. Do not place the coordinator's Work Item ID, Agent name, expected target Work Item, or answer key in the title. After repository inspection discovers the target Work Item and Assignment, register the real task or client-task ID and adopt that target repository's suggested `Work Item ID · short goal · Position (Agent Name)` title when the runtime permits renaming.
...
- Initial cold-task prompt, neutral pre-recovery title, task ID, starting revision, discovered target title, and first recovery report.
