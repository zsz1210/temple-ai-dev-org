...
13. Run the [greenfield cold-task recovery test](validation/greenfield-cold-task-recovery-test-plan.md) to validate the manual Learning Loop, UI delivery-mode selection, specification adoption, tracker mapping, bounded context recovery, task registration, and Alpha.16 runtime handoff on real work in a new Codex task. Continue Phase 2 Observer, coordination, retrieval-evaluation, and evidence-adapter work only after the remaining Phase 1.5 exit gate is complete.
