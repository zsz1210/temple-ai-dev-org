...
- `views/parallel-plan.json`: deterministic safe waves, plan-only dispatch manifests, join gates, and a source fingerprint; it creates no task or claim.
...
- `temple parallel plan` selects all active Work Items or one parent's descendants, derives dependency- and conflict-safe waves, applies an optional worker limit, and writes a generated plan unless `--no-write` is set. It never creates tasks, claims, or external actions.
- `temple work-item claim/release` records and closes Principal-backed ownership of a bounded Work Item.

Only the first wave of a fresh plan is an immediate dispatch candidate. The named Integration Owner joins candidate revisions, verification, and unresolved items; canonical changes make the plan stale, and dependent work requires replanning. See [Parallel orchestration](parallel-orchestration.md) and [ADR-0021](adr/0021-safe-group-parallel-orchestration.md).
