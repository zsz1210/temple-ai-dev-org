...
function oracleConfig(cwd) {
  const value=config();
  value.config.shell_environment_policy.set={...runtime.environment,TMPDIR:cwd};
  value.config.permissions['temple-continuity-probe'].filesystem={':minimal':'read',[cwd]:'write',[cwd+'/.git']:'write',
    ...Object.fromEntries(runtime.readRoots.slice(0,2).map(p=>[p,'read'])),glob_scan_max_depth:null};
  return value;
}
test('oracle RPC outlives native deadline and returns timeout as behavior without generation',async()=>{
  const calls=[];let closed=false;
  const providerFactory=()=>({notify(){},async request(method,params,deadline){
    calls.push(method);
    if(method==='config/read')return oracleConfig(subject.root);
    if(method==='initialize')return {};
    assert.equal(method,'command/exec');assert.equal(params.timeoutMs,20000);
    assert.ok(deadline>params.timeoutMs);assert.match(params.processId,/^continuity-oracle-/);
    assert.equal(params.permissionProfile,'temple-continuity-probe');
    return {exitCode:124,stdout:'partial output',stderr:''};
  },async close(){closed=true;}});
  assert.deepEqual(await isolatedOracleExecutor(runtime,subject.root,'/node',[],{timeout:20000,maxBuffer:100},{providerFactory}),
    {exit_code:124,stdout:'partial output',stderr:'',timed_out:true});
  assert.equal(closed,true);assert.deepEqual(calls,['initialize','config/read','command/exec']);
});
test('oracle terminates only its pending session before closing and retains uncertain cleanup',async()=>{
  for(const terminateFails of [false,true]){
    const calls=[];let owned;
    const providerFactory=()=>({notify(){},async request(method,params){
      calls.push(method);
      if(method==='initialize')return {};
      if(method==='config/read')return oracleConfig(subject.root);
      if(method==='command/exec'){owned=params.processId;throw Error('transport-timeout');}
      assert.equal(method,'command/exec/terminate');assert.deepEqual(params,{processId:owned});
      if(terminateFails)throw Error('termination-unconfirmed');return {};
    },async close(){calls.push('close');}});
    await assert.rejects(()=>isolatedOracleExecutor(runtime,subject.root,'/node',[],{timeout:20,maxBuffer:100},{providerFactory}),e=>{
      assert.equal(e.message,'transport-timeout');assert.equal(e.instrumentFailure,true);
      assert.equal(e.retainScratch,terminateFails?true:undefined);return true;
    });
    assert.deepEqual(calls,['initialize','config/read','command/exec','command/exec/terminate','close']);
  }
});
test('oracle invalid response still terminates and invalid deadlines never launch',async()=>{
  let terminated=false,closed=false;
  const providerFactory=()=>({notify(){},async request(method){
    if(method==='config/read')return oracleConfig(subject.root);
    if(method==='command/exec/terminate')terminated=true;
    return {};
  },async close(){closed=true;}});
  await assert.rejects(()=>isolatedOracleExecutor(runtime,subject.root,'/node',[],{timeout:20,maxBuffer:100},{providerFactory}),/oracle-response/);
  assert.equal(terminated,true);assert.equal(closed,true);
  for(const timeout of [undefined,0,-1,Infinity,2147483647])
    await assert.rejects(()=>isolatedOracleExecutor(runtime,subject.root,'/node',[],{timeout},{providerFactory:()=>{throw Error('must-not-launch')}}),/oracle-timeout/);
});
test('oracle RPC failure cleans an owned hanging command and child before returning',async t=>{
  const cwd=await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(),'oracle-owned-cleanup-')));
  let child,owned,pids=[];
  t.after(async()=>{if(child)try{process.kill(-child.pid,'SIGKILL');}catch{}await fs.rm(cwd,{recursive:true,force:true});});
  const script="const fs=require('fs');const c=require('child_process').spawn(process.execPath,['-e','setInterval(()=>{},1000)'],{stdio:'ignore'});fs.writeFileSync('pids.json',JSON.stringify([process.pid,c.pid]));setInterval(()=>{},1000)";
  const providerFactory=()=>({notify(){},async request(method,params){
    if(method==='initialize')return {};
    if(method==='config/read')return oracleConfig(cwd);
    if(method==='command/exec'){
      owned=params.processId;child=spawn(params.command[0],params.command.slice(1),{cwd,detached:true,stdio:'ignore'});
      for(let n=0;n<100;n++){try{pids=JSON.parse(await fs.readFile(path.join(cwd,'pids.json'),'utf8'));break;}catch{await new Promise(r=>setTimeout(r,10));}}
