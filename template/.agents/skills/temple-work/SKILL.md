---
name: temple-work
description: Operate Temple work items, evidence-bearing handoffs, workflow gates, closeout, and Codex task registration in an initialized repository. Use for ordinary Temple-managed delivery work; do not use for first-time initialization or open-ended decision interviews.
---

# Temple Work

Use Temple's CLI as the mutation boundary for supported canonical state. Do not hand-edit a work item, event stream, task registry, or generated status merely because it is faster.
...
4. After the app creates the task, register its real thread or client-thread ID with `temple task register`. Temple records task identity but never creates, renames, opens, or archives the app task by itself.
